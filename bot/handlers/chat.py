...
"
        "• свободный текст — например: <code>что думаешь по битку</code>

"
        "Нужны подробности — нажми <b>📘 Полный гайд</b>.
"
...
async def scan(m: Message):
    ...
    try:
        ...
        lines = ["🔎 <b>Scan USDT</b>", "Монеты с активностью на <code>15m / 1h / 4h</code>
"]

        total_found = 0
        for tf, items in tf_buckets:
        ...
        if total_found == 0:
            await m.answer("⚠️ Сейчас scan ничего сильного не нашёл на 15m / 1h / 4h.", parse_mode="HTML")
